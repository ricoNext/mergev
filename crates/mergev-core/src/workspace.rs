...
pub fn accept_file_side(root: &Path, path: &str, side: &str) -> Result<(), String> {
    ...
    if git_show_stage(root, stage, path).is_ok() {
        git_run(root, &["checkout", flag, "--", path])?;
        git_run(root, &["add", "--", path])?;
    } else {
        git_run(root, &["rm", "-f", "--", path])?;
    }
    Ok(())
}
...
mod tests {
    ...
    impl GitFixture {
        fn new() -> Self {
            ...
            let root = Path::new(env!("CARGO_MANIFEST_DIR"))
                ...
                .join("test-fixtures")
                .join(format!(
                    "mergev-git-fixture-{}-{unique}",
                    std::process::id()
                ));
            fs::create_dir_all(&root).unwrap();
            git_run(&root, &["init"]).unwrap();
            git_run(&root, &["config", "user.name", "mergev test"]).unwrap();
            git_run(&root, &["config", "user.email", "mergev@example.test"]).unwrap();
            fs::write(root.join("conflict.txt"), "base\n").unwrap();
            git_run(&root, &["add", "conflict.txt"]).unwrap();
            git_run(&root, &["commit", "-m", "base"]).unwrap();
            Self { root }
        }

        fn create_modify_modify_conflict(&self) {
            git_run(&self.root, &["checkout", "-b", "theirs"]).unwrap();
            fs::write(self.root.join("conflict.txt"), "theirs\n").unwrap();
            git_run(&self.root, &["commit", "-am", "theirs"]).unwrap();
            git_run(&self.root, &["checkout", "-b", "ours", "HEAD~1"]).unwrap();
            fs::write(self.root.join("conflict.txt"), "ours\n").unwrap();
            git_run(&self.root, &["commit", "-am", "ours"]).unwrap();
            assert!(git_run(&self.root, &["merge", "theirs"]).is_err());
        }

        fn create_modify_delete_conflict(&self, deleted_side: &str) {
            git_run(&self.root, &["checkout", "-b", "theirs"]).unwrap();
            if deleted_side == "theirs" {
                fs::remove_file(self.root.join("conflict.txt")).unwrap();
                git_run(&self.root, &["add", "-A"]).unwrap();
            } else {
                fs::write(self.root.join("conflict.txt"), "theirs\n").unwrap();
            }
            git_run(&self.root, &["commit", "-am", "theirs"]).unwrap();
        ...
        fn assert_no_unmerged_entries(&self) {
            assert!(
                git_stdout(&self.root, &["ls-files", "-u", "--", "conflict.txt"])
                    .unwrap()
                    .is_empty()
            );
        }
    ...
    #[test]
    fn saves_real_conflict_result_and_stages_file() {
        let fixture = GitFixture::new();
        fixture.create_modify_modify_conflict();

        let workspace = load_workspace_from_root(&fixture.root).unwrap();
        assert_eq!(workspace.files.len(), 1);
        assert_eq!(workspace.files[0].path, "conflict.txt");

        let document = load_merge_document(&fixture.root, "conflict.txt").unwrap();
        assert_eq!(document.unresolved_count, 1);

        save_merge_result(&fixture.root, "conflict.txt", "resolved\n", true).unwrap();

        assert_eq!(
            fs::read_to_string(fixture.root.join("conflict.txt")).unwrap(),
            "resolved\n"
        );
        fixture.assert_no_unmerged_entries();
        assert_eq!(
            git_stdout(
                &fixture.root,
                &["diff", "--cached", "--name-only", "--", "conflict.txt"]
            )
            .unwrap(),
            "conflict.txt"
        );
    ...
    #[test]
    fn accepts_real_file_side_and_stages_file() {
        let fixture = GitFixture::new();
        fixture.create_modify_modify_conflict();

        accept_file_side(&fixture.root, "conflict.txt", "theirs").unwrap();
    ...
    #[test]
    fn accepts_deleted_theirs_side() {
        let fixture = GitFixture::new();
        fixture.create_modify_delete_conflict("theirs");

        accept_file_side(&fixture.root, "conflict.txt", "theirs").unwrap();

        assert!(!fixture.root.join("conflict.txt").exists());
        fixture.assert_no_unmerged_entries();
    }

    #[test]
    fn accepts_deleted_ours_side() {
        let fixture = GitFixture::new();
        fixture.create_modify_delete_conflict("ours");

        accept_file_side(&fixture.root, "conflict.txt", "ours").unwrap();
    ...
    fn rejects_paths_outside_repository() {
        for path in [
            ...
            assert_eq!(
                validate_repo_relative_path(path).unwrap_err(),
                "文件路径必须是仓库内的相对路径"
            );
        }
        assert!(validate_repo_relative_path("src/nested/file.ts").is_ok());
    }

    #[test]
    fn rejects_result_with_conflict_markers() {
        let error = save_merge_result(
            Path::new("/unused"),
            "src/file.ts",
            "<<<<<<< ours\ncontent\n=======\nother\n>>>>>>> theirs\n",
            false,
        ...
        .unwrap_err();
        assert_eq!(error, "结果仍包含冲突标记，无法保存");
    }
