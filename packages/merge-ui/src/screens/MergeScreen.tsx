...
export type MergeScreenRuntime = {
	saveMergeResult: (input: {
		path: string;
		result: string;
		stage: boolean;
		repoRoot: string;
...
export function MergeScreen({
	...
	onSaved: () => void;
	runtime: MergeScreenRuntime;
}) {
	const { workspace, session } = view;
	const scrollRef = useRef<HTMLDivElement>(null);
	const highlightRequestRef = useRef(0);
	const [paneHighlights, setPaneHighlights] =
		useState<PaneHighlights>(emptyPaneHighlights);
	...
	const unresolvedConflictBlocks =
		unresolvedConflicts.length - unresolvedChanges;
	const canSave =
		session !== null &&
	...
	const saveMergeResult = runtime.saveMergeResult;
	const confirmAction = runtime.confirm;
	...
	function setDecision(
		...
		const jumpTo =
			region && isResolutionComplete(next, region)
				...
				: index;
		updateSession(rebuildSession(session.document, resolutions, jumpTo, true));
	}
	...
	// 监听来自菜单的主题切换事件
	useEffect(() => {
		if (!runtime.subscribeToThemeChanges) {
			return;
		}
		const unsubscribe = runtime.subscribeToThemeChanges((selectedTheme) => {
	...
	useEffect(() => {
		runtime.onDirtyChange?.(session?.dirty ?? false);
	}, [runtime, session?.dirty]);
