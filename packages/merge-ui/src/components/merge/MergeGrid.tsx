...
export function MergeGrid({
	...
	const renderPane = (
		...
		const contentColumns = items.reduce((maximum, { row }) => {
			const text =
				side === "ours"
					? row.oursLine?.text ?? ""
					: side === "theirs"
						? row.theirsLine?.text ?? ""
						: row.resultLine?.text ?? "";
			return Math.max(maximum, displayColumns(text));
		}, 1);
		const paneWidth =
			side === "result"
				? `calc(${contentColumns}ch + 4.35rem)`
				: `calc(${contentColumns}ch + var(--action-rail-width) + 4.35rem)`;
		return (
			<div
				className="merge-pane-lines is-virtualized"
				style={{
					height: `calc(${items.length} * var(--line-height))`,
					width: paneWidth,
					minWidth: "100%",
				}}
			>
