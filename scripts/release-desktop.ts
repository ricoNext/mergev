 ...
 *
 * 用法：
 *   bun run release:desktop 1.0.2
 *   bun run release:desktop 1.0.2 --push
 *   bun run release:desktop 1.0.2 --dry-run
 */
...
function parseArgs(argv: string[]): Args {
  ...
  if (!version) {
    fail("请传入版本号，例如: bun run release:desktop 1.0.2");
  }
...
function bumpCargoToml(version: string): void {
  ...
  if (next === content) {
    fail("未能更新 apps/desktop/src-tauri/Cargo.toml 中的 version");
  }
  write(path, next);
}
...
function bumpCargoLock(version: string): void {
  ...
  if (next === content) {
    fail("未能更新 Cargo.lock 中的 mergev version");
  }
  write(path, next);
}
...
function previousTagOrEmpty(): string {
  const tag = run("git", ["describe", "--tags", "--abbrev=0", "--match", `${TAG_PREFIX}*`], {
    allowFail: true,
  });
  return tag;
}
...
function updateChangelog(version: string): void {
  const path = "apps/desktop/CHANGELOG.md";
  const content = read(path);
  const heading = `## ${version} - ${today()}`;

  if (content.includes(`## ${version} `) || content.includes(`## ${version}\n`)) {
    fail(`apps/desktop/CHANGELOG.md 中已存在版本 ${version}`);
  }
...
function main(): void {
  ...
  const tag = `${TAG_PREFIX}${args.version}`;
  ...
  const current = JSON.parse(read("apps/desktop/package.json")).version as string;
  console.log(`当前桌面端版本: ${current} → ${args.version}`);
  ...
  run("git", ["add", ...files]);
  run("git", ["commit", "-m", `chore(desktop): release ${tag}`]);
  run("git", ["tag", tag]);
  ...
  console.log("或直接：");
  console.log(`  bun run release:desktop ${args.version} --push`);
}
